...
public class TopNavigationBar extends FrameLayout{
	...
	public TopNavigationBar(Context context, AttributeSet attrs) {
		...
		LayoutInflater.from(context).inflate(R.layout.general_navi_title_layout, this);
		...
		if(!(context instanceof NaviBtnListener)){
			DebugFlags.logD(TAG, "not enable");
		}
		if(context instanceof NaviBtnListener){
			DebugFlags.logD(TAG, "activity instanceof naviBtnListener");
			mCallBack = (NaviBtnListener)context;
		}else{
			DebugFlags.logD(TAG, "activity  not instanceof naviBtnListener");
		}
	}
	...
	public void setLeftImg(int drawableId){
		DebugFlags.logD(TAG, "setLeftImg action");
	...
	protected void onDetachedFromWindow() {
		...
		DebugFlags.logD(TAG, "onDetachedFromWindow");
		mCallBack = callBack;
	}
